...
import { MOCK_CONVERSATIONS, MOCK_USERS, getRandomReply } from '../data/mock-data.js';
...
function _loadUserProfile() {
  ...
  if (avatarEl) {
    if (user.avatar) {
      ...
      avatarEl.style.background = user.avatarGradient || 'linear-gradient(135deg,#2563EB,#7C3AED)';
...
function _renderConversationList(filter = '') {
  ...
  filtered.forEach(id => {
    ...
    div.className = 'conversation-item' + (id === activeContactId ? ' active' : '');
...
function _selectConversation(contactId) {
  ...
  if (statusEl) {
    if (conv.contact.online) {
      statusEl.innerHTML =
        `<span class="status-dot" style="position:static;width:8px;height:8px;border:none;"></span> Online`;
...
function _simulateReply() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      const replyMsg  = { id: 'm' + Date.now(), from: 'them', text: replyText, time };

      conv.messages.push(replyMsg);
      conv.lastMessage = replyText.slice(0, 30) + (replyText.length > 30 ? '…' : '');
      conv.lastTime    = time;
...
function _toggleEmojiPicker(input) {
  ...
  EMOJIS.forEach(emoji => {
    ...
    btn.style.cssText = 'width:32px;height:32px;font-size:18px;background:none;border:none;cursor:pointer;border-radius:6px;';
    btn.addEventListener('mouseover', () => { btn.style.background = 'var(--color-input)'; });
...
function _initHeaderActions() {
  document.querySelector('[aria-label="Video call"]')
    ?.addEventListener('click', () => showToast('Video call is not available in demo mode.', 'info'));
  document.querySelector('[aria-label="Voice call"]')
    ?.addEventListener('click', () => showToast('Voice call is not available in demo mode.', 'info'));
  document.querySelector('[aria-label="More options"]')
    ?.addEventListener('click', () => showToast('More options: coming soon.', 'info'));
  document.querySelector('.chat-header-actions [aria-label="Search"]')
    ?.addEventListener('click', () => showToast('In-chat search: coming soon.', 'info'));
  document.querySelector('[aria-label="Edit"]')
    ?.addEventListener('click', () => showToast('Edit mode: coming soon.', 'info'));
  document.querySelector('[aria-label="Archive"]')
    ?.addEventListener('click', () => showToast('Archive: coming soon.', 'info'));
}
...
function _initNewChatModal() {
  ...
  document.getElementById('modal-contact-search')
    ?.addEventListener('input', debounce((e) => _renderContactList(e.target.value)));
