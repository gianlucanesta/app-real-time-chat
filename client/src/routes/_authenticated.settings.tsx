    ...
    <div className="h-full flex flex-col md:flex-row bg-bg text-text-main overflow-hidden">
      ...
      <aside className="w-full md:w-[var(--width-settings-sidebar)] md:min-w-[280px] bg-bg flex flex-col p-4 md:p-6 border-b md:border-b-0 md:border-r border-border shrink-0">
        ...
        <div className="hidden md:flex items-center gap-3 mb-8">
          ...
          <span className="font-bold text-lg text-text-main tracking-wide">
            Settings
          </span>
        </div>
        ...
        <nav className="flex flex-col md:flex-col gap-0 md:gap-1.5 overflow-x-auto md:overflow-y-auto scrollbar-none pb-2 md:pb-0">
          <Link
            to="/settings"
            className="flex items-center gap-3 px-4 md:px-3.5 py-3.5 md:py-2.5 bg-input md:bg-accent/10 md:rounded-lg text-[14px] font-medium transition-colors text-accent border-b border-border md:border-b-0 rounded-t-xl md:rounded-t-lg first:rounded-t-xl shrink-0"
          >
          ...
          {[
            ...
            return (
              <a
                key={item.label}
                href="#"
                className={`flex items-center gap-3 px-4 md:px-3.5 py-3.5 md:py-2.5 bg-input md:bg-transparent md:rounded-lg text-[14px] font-medium transition-colors text-text-secondary hover:bg-input hover:text-text-main border-b border-border md:border-b-0 ${i === arr.length - 1 ? "rounded-b-xl border-b-0" : ""} shrink-0 relative`}
              >
                <Icon className="w-4 h-4" />
                <span className="flex-1">{item.label}</span>
                {item.badge && (
                  <span className="absolute right-10 md:right-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-danger"></span>
                )}
                <ChevronDown className="w-4 h-4 -rotate-90 md:hidden text-text-secondary" />
              </a>
            );
        ...
        <div className="hidden md:flex mt-auto pt-6 border-t border-border items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[14px] font-semibold text-text-main whitespace-nowrap overflow-hidden text-ellipsis">
          ...
          <button
            ...
            className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 text-text-secondary hover:text-danger hover:bg-danger/10 transition-colors"
            aria-label="Logout"
          >
      ...
      <main className="flex-1 min-w-0 overflow-y-auto scrollbar-thin px-4 md:px-12 py-6 md:py-10">
        <div className="max-w-[700px] w-full mx-auto pb-8">
          <div className="mb-8">
            <h1 className="text-[24px] md:text-[28px] font-bold text-text-main mb-1.5">
              General Settings
            </h1>
          ...
          <div className="p-6 bg-card rounded-2xl border border-border flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8 hover:border-border/80 transition-colors shadow-sm">
            ...
            <div className="flex flex-col gap-1.5 flex-1">
              <div className="text-[18px] md:text-[20px] font-bold text-text-main">
                {displayName || "Gianluca Nesta"}
              </div>
              <div className="text-[14px] text-text-secondary mb-3">
                {userRole}
              </div>
              <div className="flex gap-3">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-9 px-4 rounded-full text-[13px] font-medium border-border hover:bg-input text-text-secondary hover:text-text-main"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
            <div className="flex flex-col gap-2">
              <Label
                htmlFor="displayName"
                className="text-[12px] font-semibold text-text-secondary uppercase tracking-[0.5px]"
              >
                Display Name
              </Label>
              <Input
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                icon={
                  <MapPin className="w-[18px] h-[18px] text-text-secondary" />
                }
                className="h-11 bg-input/50 focus:bg-input rounded-xl border-border/50 text-[14px]"
              />
            </div>
            ...
            <div className="flex flex-col divide-y divide-border">
              ...
              <div className="flex flex-col gap-2 px-6 py-4 hover:bg-input/20 transition-colors">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[14px] text-accent font-semibold">
                    {volume}%
                  </span>
                </div>
                <input
          ...
          <div className="flex items-center gap-3 mt-10">
            <Button
              onClick={handleSave}
              disabled={isSaving}
              className="h-11 px-6 rounded-full font-medium shadow-md hover:shadow-lg transition-all"
            >
            ...
            <Button
              variant="outline"
              asChild
              className="h-11 px-6 rounded-full font-medium border-border hover:bg-input text-text-main"
            >
              <Link to="/">Cancel</Link>
            </Button>
            ...
            {saveMessage.text && (
              <div
                className={`flex items-center gap-2 ml-4 text-[13px] font-medium px-3 py-1.5 rounded-full ${
          ...
          <div className="mt-12 md:hidden pt-8 border-t border-border">
            <Button
              variant="outline"
              onClick={handleLogout}
              className="w-full h-11 rounded-full text-danger border-danger/20 hover:bg-danger/10 font-medium"
            >
